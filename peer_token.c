...
void *networkThread(void *param) {
	...
	while (1) {
		...
		if (recvlen > 0) {
			...
			else if(strcmp(recv_buffer, "NEW0")) {
				
				makeAddrString(send_buffer, "NEW1", &sckt->myaddr, &sckt->remaddr);
			...
			else if(strcmp(recv_buffer, "NEW1")) {
				
				strcpy(tokn_buffer, recv_buffer);
				parseMessage(tokn_buffer, &ap);
				
				if(ap.oldaddr!=sckt->myaddr.sin_addr.s_addr) {
					
					if(ap.oldaddr<sckt->myaddr.sin_addr.s_addr) {
						makeAddrString(send_buffer, "NEW1", &sckt->destaddr, &sckt->remaddr);
						sendMessage(send_buffer, sckt);
					}
					else {
						makeAddrString(send_buffer,"NEW1", &sckt->destaddr,&sckt->remaddr);
